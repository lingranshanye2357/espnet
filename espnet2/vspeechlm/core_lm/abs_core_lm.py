...
@dataclass
class VSpeechLMInferenceOptions:
    device: str = "cpu"
...
class AbsCoreLM(torch.nn.Module, ABC):
    """
    The abstract CoreLM class for VSpeechLM, which is the major component of VSpeechLM.

    It supports or is going to support several styles of VSpeechLM:
    Auto-Regressive (AR):
    ...
    For developers: to build a new core_lm model, try to follow:
        (1) Build with Espnet Espnet internal modules:
            Use modules from `espnet2.vspeechlm.module.transformer.py`. If you get
            some modules that is specific to your model, put them under
            `espnet2.vspeechlm.module.<model_name>.py`.
        (2) or, Build with HuggingFace model/modules:
            Put everyhing in `espnet2.vspeechlm.core_lm.<model_name>.py`. Usually
            this is just a warpper that bridges HF models into Espnet VSpeechLM.
    Reminder: try to avoid any model dependency beyond espnet2.vspeechlm

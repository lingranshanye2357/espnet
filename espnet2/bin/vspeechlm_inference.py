...
from espnet2.vspeechlm.definitions import SPEECHLM_TASKS, VSpeechLMTaskTemplate
from espnet2.tasks.vspeechlm import VSpeechLMTask, tokenizer_choices
...
class VSpeechLM:
    ...
    def __init__(
        self,
        task: Union[str, VSpeechLMTaskTemplate],
        ...
        """Initialize VSpeechLM module."""

        # (1) setup model
        model, train_args = VSpeechLMTask.build_model_from_file(
            train_config, model_file, device
        )
        self.model = model.to(dtype=getattr(torch, dtype)).eval()
        self.preprocessor = VSpeechLMTask.build_preprocess_fn(train_args, False)
        self.device = device
        self.dtype = dtype
        ...
        self.inference_opts = VSpeechLMInferenceOptions(
            device=device,
...
def inference(
    ...
    vspeechlm = VSpeechLM.from_pretrained(model_tag=model_tag, **vspeechlm_kwargs)

    # 3. Build data-iterator
    if (
        inference_nq is not None
        and vspeechlm.train_args.codec_token_in_use != inference_nq
    ):
        logging.warning(
            f"The model is trained with nq={vspeechlm.train_args.codec_token_in_use} "
            f"While you are inference with {inference_nq}. "
        )
        vspeechlm.train_args.codec_token_in_use = inference_nq
    if codec_ssl_corrupt_prob != vspeechlm.train_args.codec_ssl_corrupt_prob:
        vspeechlm.train_args.codec_ssl_corrupt_prob = codec_ssl_corrupt_prob
    loader = VSpeechLMTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        num_workers=num_workers,
        preprocess_fn=VSpeechLMTask.build_preprocess_fn(vspeechlm.train_args, False),
        collate_fn=VSpeechLMTask.build_collate_fn(vspeechlm.train_args, False),
                ...
                if detokenized is not None:
                    if modality in ["codec", "spk", "codec_ssl"]:
                        ...
                        torchaudio.save(
                            ...
                            sample_rate=vspeechlm.codec_tokenizer.sample_rate,
                            bits_per_sample=16,
                            encoding="PCM_S",
                        )
...
def get_parser():
    ...
    parser = config_argparse.ArgumentParser(
        description="VSpeechLM inference",
    ...
    group.add_argument(
        ...
        help="the search algorithm of VSpeechLM",
    )
    group.add_argument(
        "--nbest",
        type=int,

...
from espnet2.vspeechlm.definitions import MODALITIES, SPEECHLM_TASKS
...
import os
import sys
os.chdir('/data-03/yuyue/espnet/egs2/lrs2/vspeechlm1')
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        default='./data/token_list/llm_vocab',
        help="The input token list directory",
    )
    parser.add_argument(
        "--output_token_list_dir",
        type=Path,
        default='./data/token_list/visual_tts_lrs2',
        help="The output token list directory",
    )
    parser.add_argument(
        "--input_exp_dir",
        type=Path,
        default='exp/speechlm_tts_smollm_whole_rinit',
        help="The input experiment directory",
    )
    parser.add_argument(
        "--output_exp_dir",
        type=Path,
        default='exp/vspeechlm_visual_tts_lrs2_pretrain_delay_tts',
        help="The output experiment directory",
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default='31_40epoch.pth',
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="+",
        default=[Path('/data-03/yuyue/espnet/egs2/lrs2/vspeechlm1/dump/raw_visual_tts_lrs2/train/token_lists/video_ssl_token_list')],
...
def main():
    ...
    token_list = json.load(open(token_list))  # 59670
    ...
    config = yaml.safe_load(open(config))  # token_list: 59670
    ...
    print(token_list_dict["g2p_'"])  # the last one
    token_list_dict["g2p_,"] = len(token_list)  # 59670  应该加在新vocab的前面才对
    num_new_tokens = 1
    # 扩展g2p tensor
    for tensor_name in [
        "corelm.emb.weight",
        ...
        old_tensor = checkpoint[tensor_name]  # [59670, 960]  [59670, 960]
    ...
    token_list = list(token_list_dict.keys())  # 59670

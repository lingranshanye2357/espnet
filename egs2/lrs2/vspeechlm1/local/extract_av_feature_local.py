...
def per_file(f_name, video_path, args, video_process, model):
    # f_name: video_id
    ...
    if os.path.exists(os.path.join(args.landmark_path, f_name + ".pkl")):
        ...
        feats = feats.squeeze(0).transpose(0, 1).contiguous().cpu().numpy()
        print(f"debug feats1: {feats.shape}")
        np.save(os.path.join(args.output_dir, f_name + '.npy'), feats)
        # 35, 2048
...
def main():
    ...
    feat_file = open(args.feat_file, 'w')
    import tqdm
    for f in tqdm.tqdm(file_lists):
        f_name, video_path = f
        if os.path.exists(os.path.join(args.output_dir, f_name + '.npy')):
            feat_file.write(f"{f_name} {os.path.join(args.output_dir, f_name + '.npy')}\n")
            continue
        per_file(f_name, video_path, args, video_process, model)


def get_parser():
    parser = argparse.ArgumentParser(description="Command-line script for preprocessing.")
    parser.add_argument("--file_list", type=str, default='/nfs-02/yuyue/visualtts/reference_code/espnet/egs2/lrs2/speechlm1/data/all.txt', help="file_list (scp)")
    ...
    parser.add_argument("--landmark_path", type=str, default="./local/landmark", help="path including landmark files",)
    parser.add_argument("--write_num_frames", type=str, help="specify wspecifer for num frames")
    parser.add_argument("--output_dir", type=str, help="path to save avhubert feature")
    parser.add_argument("--feat_file", type=str, help="path to save the feat.scp")
    return parser
